...
def webhook():
    ...
    req = request.get_json(silent=True, force=True)            # read entire json where user name, mail, phone is there
...
def processRequest(req):   #req is entire json
    log = logger.Log()

    sessionID=req.get('responseId')


    result = req.get("queryResult")   #extract interested part  name,mail,phone
    ...
    intent = result.get("intent").get('displayName')  #extract intent name
    if (intent=='course_selection'):   # we got all info for sending mail
